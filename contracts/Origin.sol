...
contract Origin {
    ...
    function addProduct(string memory _name, string memory _process, string memory _date) public onlyOwner {
        require(bytes(_name).length != 0);
        require(bytes(_process).length != 0);
        require(bytes(_date).length != 0);
        productCount++;
        products[productCount] = Product(productCount, _name, _process, _date, msg.sender);
        emit ProductAdded(productCount, _name, _process, _date, msg.sender);
    }
